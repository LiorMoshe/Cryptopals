...
class Sha1Hash(object):
    ...
    def __init__(self,internal_state = None,message_byte_length = None):
        ...
        if (internal_state == None):
            ...
            self._h = internal_state

        # bytes object with 0 <= len < 64 used to store the end of the message
        # if the message length is not congruent to 64
        self._unprocessed = b''
        # Length in bytes of all data that has been processed so far
        if (message_byte_length == None):
...
def sha1(data,internal_state = None,message_byte_length = 0):
    ...
    """
    return Sha1Hash(internal_state,message_byte_length).update(data[message_byte_length:]).hexdigest()
